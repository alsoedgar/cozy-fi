...
- **Spotify app / browser** does not start the local player. Premium listeners can select an active Spotify device and still control playback, seeking, volume, and the Cozy-Fi queue from this app; Free or restricted accounts use Spotify links until an eligible device is available.
...
Spotify's public API has no queue removal or reordering endpoint. Cozy-Fi therefore manages the playback list for sessions started here, including sessions controlled through Spotify App mode. Add, remove, reorder, and shuffle change only the upcoming list, so the playing song continues without pausing, restarting, or rebuffering; Cozy-Fi applies the edited order at the next song boundary. Choosing **PLAY**, **NEXT**, or **PREVIOUS** changes songs immediately as expected. Native playlist playback is used until the edited order needs to take over. Longer editable lists use rolling playback windows so the API's 100-track request size does not truncate the rest of the session. Playlist loading is bounded at 10,000 items; library browsing still displays up to 500. Queues started elsewhere are available as a read-only view until Cozy-Fi adopts the active Premium track, after which add, remove, reorder, shuffle, and play-position controls are available here.
...
- A synchronized result follows playback, centers the current line, and marks it with a high-contrast **NOW** treatment. In the Side Player's standalone mode or controllable Premium Spotify App mode, select any timestamped lyric to jump playback directly to that line. Scroll manually at any time; select **FOLLOW** to return to the current line.
...
- Automatic line following uses Cozy-Fi's playback clock in standalone mode and in Premium Spotify App mode when an active device is controllable. Link-only Spotify App mode keeps synchronized lyrics available for manual scrolling because playback timing remains controlled by Spotify.
...
Open **MENU → Side Player** to switch into the compact player; the full window hides so only the side player remains. Its live Spotify artwork, timeline, play/pause, previous, and next controls use the same playback session as the full app. Select **PLAYER** or **LYRICS** in the compact header to switch between two focused modes. PLAYER restores the artwork and all compact playback controls. LYRICS uses the full Side Player body for a cleaner, larger reading view while keeping the mode switch and window actions visible. Timestamped LRCLIB or local `.lrc` lines are selectable during standalone playback or controllable Premium Spotify App mode and jump directly to that point; plain untimed lyrics and link-only Spotify App mode remain scroll-only. Use **LOCAL** to add, replace, or remove a local lyric file without returning to the full app. Drag the lower-right grip to resize it, or focus the grip and use the arrow keys. **PIN** toggles always-on-top, **FULL** (or a title-bar double-click) hides the compact window and restores the full app, and **HIDE** minimizes the side player so it can be restored from the taskbar or dock. Its size and screen position are remembered.

Morning Lo-Fi, Soft Sunset, Cover Match, Cozy Glass, enlarged type, and custom palette previews are mirrored into the side player immediately. Standalone mode and Premium Spotify App mode provide the compact transport, timeline, and queue controls; Free or restricted Spotify App mode shows the selected track with an **OPEN** action until Spotify provides an eligible active device.

Spotify removed the `product` subscription field from `GET /me` for Development Mode apps in February 2026, so Cozy-Fi cannot always read a trustworthy Free/Premium label directly. It uses capability detection instead and never treats an ordinary network error as proof that an account is Free. Spotify's current [Web API requirements](https://developer.spotify.com/documentation/web-api) also require Premium; Premium Spotify App mode remains controllable from Cozy-Fi, while Free or restricted accounts use the graceful link fallback.
...
For an easy Windows install, build the unpacked app and run `npm run installer:windows`. The installer is written to `dist/installer-win32-x64/Cozy-Fi-Setup.exe` and creates a Start Menu/Desktop shortcut through the normal Windows installer flow. Version tags build the platform downloads, Windows installer, and `SHA256SUMS.txt` automatically in [GitHub Releases](https://github.com/alsoedgar/cozy-fi/releases/latest).
...
- Version 1.6.3 makes queue editing non-interrupting. Add, remove, reorder, and shuffle update the upcoming list without restarting or pausing the current song; the edited order takes effect at the next song boundary.
